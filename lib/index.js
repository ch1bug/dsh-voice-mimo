...
import { applyStyle, resolveStyle, resolveTtsTarget, truncateTtsText, DEFAULT_STYLE } from "./tts.js";
import {
  cleanTmp,
...
const Config = z.object({
  ...
  tts: z
    .object({
      ...
      /** Read-aloud style (朗读语气): natural-language instruction, default 温柔. */
...
async function apply(ctx, config) {
  ...
  const safeConfig = {
    ...
    tts: {
      ...
      style: (tts.style && tts.style.trim()) || DEFAULT_STYLE,
    },
  };
  ...
  try {
    ...
    // Issue #5 — startup sweep: enforce the loose retention fallback and
    // clean long/ audio of already-archived sessions. The live
    // host/archived-sessions-changed frame only reaches the client runtime,
    // so the client half drives the incremental cleanup via HTTP; this sweep
    // (reading ctx.workspaceRegistry.archivedSessionIds directly) catches
    // anything that happened while the GUI was closed.
    try {
      ...
      if (retention.removed > 0) {
        ctx.logger?.info?.(`[dsh-voice-mimo] retention removed ${retention.removed} long-term artifacts`);
      }
      ...
      for (const sessionId of archived) {
        ...
        if (outcome.removed > 0) {
          ctx.logger?.info?.(`[dsh-voice-mimo] startup sweep cleaned ${outcome.removed} artifact(s) of archived session ${sessionId}`);
        }
      }
    } catch (error) {
      ctx.logger?.warn?.(
        `[dsh-voice-mimo] startup sweep failed (retention/archived cleanup will retry on demand): ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  } catch (error) {
    ctx.logger?.warn?.(
  ...
  if (current().stt.enabled) ctx.tools.register(
    defineTool({
      ...
      description:
        "Understand the content of an audio file (wav/mp3/m4a/ogg/flac) with the Xiaomi MiMo multimodal model: summarize what was said, extract information, identify speakers/emotion, or answer questions about the audio. Unlike voice_transcribe (verbatim transcript), this returns a semantic analysis. IMPORTANT: the audio model CANNOT see the conversation history — always put the relevant background and your extraction goal into `prompt` (e.g. '用户刚才在讨论X，这段录音是Y，请提取Z'), or pass the background in `context`. Use this when the user wants to know what an audio is ABOUT, not its exact wording.",
      parameters: {
        path: {
          type: "string",
          required: true,
        ...
        prompt: {
          type: "string",
          description: "What to find out about the audio (e.g. '总结要点', '这是什么情绪', '提取人名'), including the conversation background the model cannot see. Default: a detailed summary.",
        },
        context: {
          type: "string",
          description: "Optional conversation background injected into the system prompt as a 背景 section — the audio model does not see the chat history, so relevant context belongs here or in `prompt`.",
        },
      },
      output: {
        schema: {
      ...
      async execute(args, exec) {
        ...
        const apiKey = await resolveMimoKey(ctx);
        const wsl = wslPathOf(args.path);
        const prompt = args.prompt || "Listen to this audio and provide a detailed summary of its content, main points, and any notable details. If background context was provided, relate the audio to it (which conversation point it addresses).";
        const tmp = `/tmp/voice_mimo_und_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
        // #11: the audio model cannot see the conversation history, so the
        // agent-passed `context` rides the system prompt as a 背景 section
        // (system travels via argv + shq — not inlined into the python source).
        const ctxText = (typeof args.context === "string" && args.context.trim()) ? args.context.trim() : "";
        const system = "You are MiMo, an AI assistant developed by Xiaomi. When the user provides an audio part, listen to it and answer about its content."
          + (ctxText ? ` Background context (对话背景): ${ctxText}. Relate the audio to this background when relevant.` : "");
        const py = b64([
          "import sys,json,base64,urllib.request,urllib.error",
          "src,system,prompt,api,url,out = sys.argv[1:7]",
          "b=open(src,'rb').read()",
          "durl='data:audio/wav;base64,'+base64.b64encode(b).decode()",
          "payload={'model':'mimo-v2.5','messages':[{'role':'system','content':system},{'role':'user','content':[{'type':'input_audio','input_audio':{'data':durl}},{'type':'text','text':prompt}]}],'stream':False,'max_completion_tokens':4096}",
          "def attempt():",
          "  req=urllib.request.Request(url,data=json.dumps(payload).encode(),headers={'api-key':api,'Content-Type':'application/json'})",
        ...
        const pyCmd = `printf '%s' ${py} | base64 -d | python3 - ${shq(wsl)} ${shq(system)} ${shq(prompt)} ${shq(apiKey)} ${shq(baseUrl + "/chat/completions")} ${shq(tmp + ".resp")}`;
  ...
  if (current().tts.enabled) ctx.tools.register(
    defineTool({
      ...
      parameters: {
        ...
        voice: {
          type: "string",
          description: "Optional voice: an OpenAI voice name from the configured voice map (alloy/echo/fable/onyx/nova/shimmer), a MiMo preset (mimo_default/冰糖/茉莉/苏打/白桦/Mia/Chloe/Milo/Dean), or a free-form Chinese voice description.",
        },
        reference: {
          type: "string",
        ...
        style: {
          type: "string",
          description: "Optional speaking style (语气): a natural-language instruction such as 温柔/沉稳/轻快, or a full director-style paragraph (角色/场景/指导). Default: Settings tts.style (温柔). With voicedesign voices the style becomes an inline (风格) tag prefix.",
        },
        sing: {
          type: "boolean",
          description: "Sing the text (preset voices only, mimo-v2.5-tts): the text is prefixed with a bare (唱歌) tag — put flowing, melodic Chinese lyrics in `text` (a single flowing line reads better than repeated short lines). `style` is not applied when singing.",
        },
      },
      output: {
        schema: {
          ...
          properties: {
            ...
            style: { type: "string", description: "The style instruction applied (resolved default when not passed)." },
            truncated: { type: "boolean", description: "True when the text exceeded 2500 chars and was cut." },
          },
          additionalProperties: false,
        },
        render: (_args, value) => {
          const blocks = [
            { type: "text", text: `Generated speech: ${value.path} (${value.bytes} bytes${value.seconds != null ? `, ${value.seconds}s` : ""}${value.style ? `, style: ${value.style}` : ""})${value.truncated ? " [text truncated at 2500 chars]" : ""}` },
          ];
      ...
      async execute(args, exec) {
        ...
        const style = resolveStyle(cfg, args.style);
        ...
        // Style/sing (spec #6): apply tags first, then truncate the FINAL text
        // so the (唱歌 style)/(style) prefix can never push it past the API
        // limit. applyStyle throws when singing is requested with a non-preset
        // voice — fail fast, before any API call.
        ...
        const pyCmd = `printf '%s' ${py} | base64 -d | python3 - ${shq(model)} ${shq(textToSpeak)} ${shq(userContent)} ${shq(JSON.stringify(audio))} ${shq(referencePath || "")} ${shq(apiKey)} ${shq(baseUrl + "/chat/completions")} ${shq(tmp + ".resp")}`;
        const r = await run(ctx, pyCmd, exec, { timeoutMs: cfg.timeoutMs || 200000 });
        const status = r.stdout.text.trim();
        if (!status.startsWith("OK")) {
          let errText = "";
        ...
        if (plan.manifest) {
          ...
          try {
            ...
            // Loose retention fallback (issue #5): after every long-term write,
            // bound the store by count/days so an unarchived history cannot
            // grow the disk forever. Idempotent and cheap at the 200 cap.
            const audio = current().audio ?? {};
        ...
        const result = { path: plan.path, bytes: buf.length, notify, style };
