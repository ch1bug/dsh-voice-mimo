...
/** Convert a Windows path (C:\...) or WSL path to a WSL path. */
...
async function apply(ctx, config) {
  ...
  if (current().stt.enabled) ctx.tools.register(
    defineTool({
      ...
      async execute(args, exec) {
        ...
        return { text, language: args.language || undefined };
      },
    }),
  );
    ...
    defineTool({
      ...
      async execute(args, exec) {
        ...
        const py = b64([
          ...
          "b=open(src,'rb').read()",
          "durl='data:audio/wav;base64,'+base64.b64encode(b).decode()",
          "payload={'model':'mimo-v2.5','messages':[{'role':'user','content':[{'type':'input_audio','input_audio':{'data':durl}},{'type':'text','text':prompt}]}],'stream':False,'max_completion_tokens':4096}",
          "req=urllib.request.Request(url,data=json.dumps(payload).encode(),headers={'api-key':api,'Content-Type':'application/json'})",
          "try:",
  ...
  if (current().tts.enabled) ctx.tools.register(
    defineTool({
      name: "voice_speak",
      description:
        "Generate a spoken audio file from text using the Xiaomi MiMo TTS models. Writes a .wav file (Windows side by default) and returns its path. To read a reply aloud directly in the Web UI, use the 🔊 button on the message instead.",
      parameters: {
        text: {
          type: "string",
          required: true,
        ...
        outPath: {
          type: "string",
          description: "Optional output .wav path on the Windows side (default: C:\\Windows\\Temp\\voice-mimo-<ts>.wav).",
        },
        voice: {
          type: "string",
      ...
      async execute(args, exec) {
        ...
        const baseUrl = (provider.baseUrl || "https://api.xiaomimimo.com/v1").replace(/\/+$/, "");
        const apiKey = await resolveMimoKey(ctx);
        const outPath = args.outPath || `C:\\Windows\\Temp\\voice-mimo-${Date.now()}.wav`;
        ...
        const mapped = current().voiceMap?.[voiceName];
        let model = cfg.model || "mimo-v2.5-tts";
        let userContent;
        let audio;
        ...
        if (mappedModel.includes("voiceclone")) {
        ...
        } else if (mapped !== undefined && mapped.type === "voicedesign") {
          // Explicit model override wins; otherwise infer from type.
          ...
          if (!PRESET_VOICES.has(preset)) {
            ...
            if (mapped?.model) model = mapped.model;
        ...
        let bytes = 0;
        try { bytes = Number((await run(ctx, `wc -c < ${shq(outWsl)}`, exec, { timeoutMs: 5000 })).stdout.text.trim()) || 0; } catch {}
        return { path: outWsl, bytes };
      },
    }),
  );
