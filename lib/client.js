...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SpeakerButton(props) {
      ...
      // Stop whatever else is playing WITHOUT letting the notify loop reset
      // this button's own state (we assert our own state right after).
      const stopOthers = () => {
        const self = stopHandlerRef.current;
        if (self) audioStopListeners.delete(self);
        try {
          stopActiveAudio();
        } finally {
          if (self) audioStopListeners.add(self);
        }
      };
      ...
      const playAudio = (url) => {
        ...
        audioRef.current = a;
        activeAudio = a;
        a.onended = () => { setState("idle"); activeAudio = null; };
        a.onerror = () => {
          setState("idle");
          activeAudio = null;
