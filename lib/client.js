...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SpeakerButton(props) {
      ...
      // current session snapshot (nodes live under `chat.nodes`). Calling it
      // without a selector crashes useSyncExternalStoreWithSelector.
      const nodes = typeof useSession === "function" ? useSession((s) => s?.chat?.nodes) : null;
