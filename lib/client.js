...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function VoiceSpeakView(props) {
      ...
      // Issue #5 — the file may have been cleaned by an archived session /
      // retention while the manifest entry (params) survives. probe:
      ...
      useEffect(() => {
        ...
        let alive = true;
        const probeId = (envelope.audioUrl || "").split("/").pop();
        setProbe("unknown");
        ...
        return () => { alive = false; };
      }, [settled, envelope ? (envelope.audioUrl || "").split("/").pop() : null]);

      // Issue #5 — re-synthesize a cleaned artifact from its manifest
      // parameter record. The host rewrites the SAME id, so the strip's
      // audioUrl stays valid and the play path just works again.
      const regenerate = () => {
        if (regenerating || !props.sessionId || !props.callId) return;
        setRegenerating(true);
        logVoice("regenerate-request", { sessionId: props.sessionId, callId: props.callId });
        fetch("/_dsh/voice-mimo/regenerate", {
          method: "POST",
          credentials: "same-origin",
          headers: { "Content-Type": "application/json" },
          ...
          .then((d) => {
            ...
            logVoice("regenerate-ok", { id: d.value.id, bytes: d.value.bytes });
            setProbe("ok");
          })
          .catch((err) => {
            logVoice("regenerate-error", { message: err && err.message });
            if (typeof window !== "undefined") window.alert("重新生成失败：" + ((err && err.message) || err));
          })
      ...
      if (probe === "cleaned" || probe === "missing") {
        // Issue #5 — the artifact was cleaned (archived session / retention);
        // the manifest entry still carries the synthesis parameters, so a
    ...
    function apply(ctx) {
      ...
      // Issue #5 — archive cleanup. The `host/archived-sessions-changed`
      // frame reaches the CLIENT runtime only (api-proxy pushes it to client
      // mux queues, not the host cordis bus), so this half diffs the full
        ...
        try {
          ...
          if (ws && ws.list && typeof ws.list.subscribe === "function") {
            ...
            const onChange = () => {
              ...
              logVoice("archive-cleanup", { sessionIds: added });
              fetch("/_dsh/voice-mimo/archive-cleanup", {
                method: "POST",
                credentials: "same-origin",
                headers: { "Content-Type": "application/json" },
                ...
                .then((r) => r.json())
                .then((d) => {
                  if (!d.ok) logVoice("archive-cleanup-error", { code: d.error && d.error.code, message: d.error && d.error.message });
                })
                .catch((e) => logVoice("archive-cleanup-error", { message: e && e.message }));
            };
            dispose = ws.list.subscribe(onChange);
          }
        } catch (error) {
          logVoice("archive-watch-error", { message: error && error.message });
        }
        return () => {
          if (dispose) { try { dispose(); } catch (_) { /* ignore */ } }
        };
      }, "dsh-voice-mimo: archived-session cleanup watcher");
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            // Options mirror the voice map rows below, labelled with the
            // resolved MiMo voice (e.g. "alloy（冰糖）"). A selected value whose
            // row was deleted gets a visible fallback instead of vanishing.
            (() => {
              const voiceMap = state.draft.voiceMap || {};
              const current = (state.draft.tts && state.draft.tts.voice) || "alloy";
              const opts = Object.entries(voiceMap).map(([name, entry]) =>
                React.createElement("option", { key: name, value: name },
                  `${name}（${entry && entry.type === "voicedesign" ? "设计音色" : ((entry && entry.voice) || "?")}）`));
              if (!voiceMap[current]) {
                opts.unshift(React.createElement("option", { key: `orphan-${current}`, value: current }, `${current}（未在映射中）`));
              }
              return opts;
            })(),
          ),
