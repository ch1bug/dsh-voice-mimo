...
import { applyStyle, DEFAULT_READ_ALOUD_VOICE, DEFAULT_STYLE, resolveStyle, resolveTtsTarget, speakMiMo, truncateTtsText } from "./tts.js";
...
export const REGENERATE_ROUTE = "/_dsh/voice-mimo/regenerate";
export const ARCHIVE_CLEANUP_ROUTE = "/_dsh/voice-mimo/archive-cleanup";
 ...
 * manifest entry, and return { id, audioUrl, bytes, voice, model, style }.
 *
 * Voice resolution: optional explicit `voice` (forward-compatible with
 * regenerate), else Settings `tts.voice` (朗读音色), else "alloy". The style
 * (explicit `style` param, else Settings `tts.style` 朗读语气, else 温柔) is
 * applied through the shared mixed channel (applyStyle) and truncated to the
 * shared 2500 limit. The manifest entry carries {text, voice, model, style}.
 */
export async function performSpeak(ctx, getSettings, { text, voice, style } = {}, deps = {}) {
  ...
  // Read-aloud style (spec #6 / issue #8): explicit request `style` wins,
  // else Settings tts.style (朗读语气), else the shared default. Applied via
  // the same mixed channel as voice_speak (preset/voiceclone → user message,
  // voicedesign → (style) tag prefix); text is truncated to the shared limit.
  const styleName = resolveStyle(tts, style);
  // Apply tags FIRST, then truncate the final text — mirrors the voice_speak
  // path: the (style) tag prefix must never push the payload past the shared
  // 2500-char limit.
  ...
  const entry = await manifestAppend(dir, { id, rel, text: t, voice: voiceName, model: target.model, style: styleName });
  const audioUrl = `${AUDIO_PREFIX}/${id}`;
  const result = { id, audioUrl, bytes: bytes.length, voice: voiceName, model: target.model, style: styleName, createdAt: entry.createdAt };
...
export async function speakHttp(ctx, getSettings, body, deps = {}) {
  ...
  try {
    const result = await performSpeak(ctx, getSettings, { text, voice: body.voice, style: body.style }, { fetchImpl: deps.fetchImpl });
...
/**
 * POST /regenerate {sessionId, callId} (issue #5): restore a cleaned long/
 * artifact from its manifest parameter record. Re-synthesizes via MiMo with
 * the stored {text, voice, model, style, sing}, writes back under the SAME id
 * (the message strip's audioUrl stays valid — no client URL swap), and
 * appends a fresh manifest line so the loose retention sees the new
 * createdAt. The original manifest line stays as history; lookups prefer the
 * latest line per id.
 *
 ...
 */
export async function performRegenerate(ctx, getSettings, { sessionId, callId } = {}, deps = {}) {
...
export async function regenerateHttp(ctx, getSettings, body, deps = {}) {
  try {
    const result = await performRegenerate(ctx, getSettings, { sessionId: body?.sessionId, callId: body?.callId }, { fetchImpl: deps.fetchImpl });
    return { status: 200, json: { ok: true, value: result } };
  } catch (error) {
...
/**
 * POST /archive-cleanup {sessionIds} (issue #5): the host half of the
 * archived-sessions event. The `host/archived-sessions-changed` frame only
 * flows to the client runtime (api-proxy pushes it to client mux queues, not
 * the host cordis bus), so the client half diffs the full archivedSessionIds
 * set and drives this route; the host also sweeps `ctx.workspaceRegistry`
 * archived ids at startup as a robustness fallback. Cleans each session's
 * long/ audio only — manifest lines stay (regenerate params), session logs
 * are never touched.
 */
export async function performArchiveCleanup(ctx, getSettings, sessionIds) {
...
export async function archiveCleanupHttp(ctx, getSettings, body) {
  ...
  try {
    const result = await performArchiveCleanup(ctx, getSettings, body.sessionIds);
    return { status: 200, json: { ok: true, value: result } };
  } catch (error) {
...
export function installVoiceMimoWeb(ctx, getSettings, updateSettings) {
  ctx.inject(["webServer"], (webCtx) => {
    webCtx.effect(() => {
      ...
      const audioDispose = webCtx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (req.method !== "GET" && req.method !== "HEAD") {
            res.writeHead(405, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ ok: false, error: { code: "method-not-allowed", message: "Use GET or HEAD" } }));
            return;
          }
      ...
      const regenerateDispose = webCtx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const { status, json } = await regenerateHttp(ctx, getSettings, body);
          res.writeHead(status, { "Content-Type": "application/json" });
          res.end(JSON.stringify(json));
        },
      });
      const archiveCleanupDispose = webCtx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          const { status, json } = await archiveCleanupHttp(ctx, getSettings, body);
          res.writeHead(status, { "Content-Type": "application/json" });
          res.end(JSON.stringify(json));
        },
      });
    ...
    }, "voice-mimo: settings + import + log + transcribe + speak + audio + regenerate + archive-cleanup routes");
  });
}
