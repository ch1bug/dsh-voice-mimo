...
/** Validate an audio id path segment; throws on anything unexpected. */
 ...
 * Returns { path, bytes } or throws (404 semantics decided by the caller).
 */
export async function performAudio(ctx, getSettings, rawId) {
  ...
  if (!entry) throw new Error(`audio ${id} not found`);
  const path = entryAbsolutePath(dir, entry);
  const info = await stat(path).catch(() => null);
  if (!info || !info.isFile()) throw new Error(`audio ${id} missing on disk`);
      ...
      const speakDispose = webCtx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            for await (const chunk of req) chunks.push(chunk);
            const body = JSON.parse(Buffer.concat(chunks).toString("utf8") || "{}");
            const result = await performSpeak(ctx, getSettings, { text: body.text, voice: body.voice });
            respond({ ok: true, value: result });
          } catch (error) {
            respond({ ok: false, error: { code: "speak-error", message: error instanceof Error ? error.message : String(error) } }, 500);
          }
        },
      });
      const audioDispose = webCtx.webServer.register({
        ...
        handler: async (req, res) => {
          try {
            ...
            res.end(JSON.stringify({ ok: false, error: { code: "audio-not-found", message: error instanceof Error ? error.message : String(error) } }));
          }
        },
      });
      return () => {
